...
namespace Microsoft.Framework.PackageManager
{
    public class WrapCommand
    {
            ...
            bool isSubProcedure = false)
        {
            ...
            var relativePdbPath = PathUtility.GetRelativePath(targetProjectJson, pdbPath, PathSeparator);
            ...
            var dependencyAssemblies = assemblyInfo.GetDependencies().Select(x => x + ".dll");

            foreach (var filePath in Directory.EnumerateFiles(Path.GetDirectoryName(assemblyPath)))
            {
                var fileName = Path.GetFileName(filePath);
                if (dependencyAssemblies.Any(x => string.Equals(x, fileName, StringComparison.OrdinalIgnoreCase)))
                {
                    EmitAssemblyWrapper(wrapRoot, targetFramework, filePath, isSubProcedure);
                    AddProjectDependency(projectJson, Path.GetFileNameWithoutExtension(filePath), targetFramework);
                }
            }
